class Key{
  ...
  void key(){
    if(player.move_directionY==0){
      if(key_up==1 && maps.hash(player.about_chipX()+1,player.about_chipY())!=color(0)){
        player.move_directionY=1;
        player.player_direction=1;
      }
      if(key_down==1 && maps.hash(player.about_chipX()+1,player.about_chipY()+2)!=color(0)){
        player.move_directionY=2;
        player.player_direction=2;
      }
    }
    if(player.move_directionX==0){
      if(key_left==1 && maps.hash(player.about_chipX(),player.about_chipY()+1)!=color(0)){
        player.move_directionX=1;
        player.player_direction=3;
      }
      if(key_right==1 && maps.hash(player.about_chipX()+2,player.about_chipY()+1)!=color(0)){
